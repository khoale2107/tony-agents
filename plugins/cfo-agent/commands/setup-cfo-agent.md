...
4. **Thu thập cấu hình.** Tạo file `.env` từ `.env.example` và điền giúp. Hỏi học viên lần lượt (giải thích chỗ lấy từng thứ):
...
5. **Gửi thử một báo cáo thật** (nếu học viên đã điền đủ Telegram): chạy `./run.sh` (hoặc `.\run.ps1`) và xác nhận họ nhận được tin nhắn. Phần nhận định lần đầu có thể mất ~15-30s vì gọi Claude Code.

6. **Đặt lịch 7h sáng.** Hỏi giờ mong muốn (mặc định 7:00), rồi chạy đúng script theo hệ điều hành:
...
7. **Tổng kết** cho học viên: agent nằm ở đâu, sửa số liệu ở Google Sheet nào, đổi cấu hình ở file `.env` nào, và cách chạy tay để kiểm tra.
