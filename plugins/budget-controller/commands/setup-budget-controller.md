---
description: Dựng Budget Controller — Cảnh báo ngân sách theo phòng ban: so chi thực tế với ngân sách tháng, cảnh báo 
---

Bạn đang giúp học viên khóa "50 AI Agents" của Tony Academy dựng **Budget Controller** vào thư mục hiện tại. Chạy qua GÓI Claude Code (không cần API key). Nói tiếng Việt, ngắn gọn, tự làm giúp thao tác.
...
3. **Chạy thử ngay:** `cd <thư mục> && ./run.sh --dry-run   # xem thử` (Windows: đổi `./run.sh` -> `.\run.ps1`). Cho học viên xem kết quả.
